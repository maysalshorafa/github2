                    ...
                    for (Product p : d.getProducts()) {
                        ...
                        if(availableProductName&&availableBarCode) {
                            productDBAdapter.insertEntry(p.getProductCode(), p.getBarCode(),
                                    ...
                                    p.getSku(), p.getStatus(), p.getDisplayName(), p.getRegularPrice(), p.getStockQuantity(), p.isManageStock(), p.isInStock(),p.getUnit(),p.getWeight(),p.getCurrencyType(),p.getBranchId(),0);
